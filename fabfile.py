...
def development():
    env.env = 'development'
    env.settings = 'beavlet.settings.development'


def staging():
    env.env = 'staging'
    env.settings = 'beavlet.settings.staging'
    env.remote = ''
    env.heroku_app = ''
...
def production():
    env.env = 'production'
    env.settings = 'beavlet.settings.production'
    env.remote = ''
    env.heroku_app = ''
...
def brunchwatch(app_name='core'):
    local('cd beavlet/%s/static/ && brunch w' % app_name)


def brunchbuild(app_name='core'):
    with settings(warn_only=True):
        local('rm -r beavlet/%s/static/public/' % app_name)
    local('cd beavlet/%s/static/ && brunch b -m' % app_name)
